  ...
  static async getQuizzes(
    ...
    try {
      ...
      if (categoryId && difficulty) {
        ...
        quizQuery = query(quizzesRef, orderBy('createdAt', 'desc'));
      }

      // Apply pagination
      if (lastQuizDoc) {
        quizQuery = query(quizQuery, startAfter(lastQuizDoc), limit(pageSize));
      } else {
        quizQuery = query(quizQuery, limit(pageSize));
      }
  ...
  static async getQuestionSummaries(
    ...
    try {
      ...
      if (categoryId && difficulty) {
        ...
        questionsQuery = query(questionsRef, orderBy('createdAt', 'desc'));
      }

      // Apply pagination
      if (lastDoc) {
        questionsQuery = query(questionsQuery, startAfter(lastDoc), limit(pageSize));
      } else {
        questionsQuery = query(questionsQuery, limit(pageSize));
      }
