...
/**
 * Script to add additional quiz data to Firestore
 * 
 * This script reads quiz data from additional-quizzes.json and adds it to Firestore
 * Run with: node scripts/db/add-additional-quizzes.js
 */
...
const quizDataPath = path.resolve(__dirname, 'additional-quizzes.json');
let quizData;
...
async function main() {
  try {
    ...
    console.log('All additional data has been successfully added to Firestore!');
    process.exit(0);
  } catch (error) {
